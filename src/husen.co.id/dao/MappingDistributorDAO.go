...
func GetMappingNexseller(db *sql.DB, ncId int64, data *model.MappingNexseller) (err error) {
	query :=
		"SELECT 'mn', id FROM mapping_nexseller where nexchief_account_id = $1 AND code = $2 " +
			...
			"UNION ALL "
	tempQuery, tempParam := GetQueryParent("geo_tree", "gt", "code", nil, []interface{}{
...
func InsertMappingNexseller(db *sql.DB, ncID int64, data *model.MappingNexseller) (err error) {
	query :=
